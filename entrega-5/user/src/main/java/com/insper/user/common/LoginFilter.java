...
public class LoginFilter implements Filter {
    ...
    List<String> postOpenRoutes = Arrays.asList("/login");
    ...
    public void doFilter(
        ...
        if (method.equals("GET") && (openRoutes.contains(uri) || uri.startsWith("/login/token/"))) {
            chain.doFilter(request, response);
        } else if (method.equals("POST") && postOpenRoutes.contains(uri)) {
            chain.doFilter(request, response);
        } else {
